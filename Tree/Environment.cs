...
namespace Tree
{
    public class Environment : Node
    {
		...
		//added stuff from here and down
		//obtaining value of a node
        ...
        public void assign(Node id, Node value)
        {
            Node node = Environment.find(id, this.frame);
			if (node == null && this.env == null)
				Console.Error.WriteLine("undefined variable " + id.getName());
			else if (node == null)
				this.env.assign(id, value);
			else
				node.setCar(value);
        }
    }
